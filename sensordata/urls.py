...
router.register(r'val', rest.DataValueList)
...
urlpatterns = patterns('',
    ...
    #url(r'^api/datavalue/sn/(?P<serial_number>[a-zA-Z0-9-_\.]+)/$', rest.DataValueForDevDetail.as_view()),
    url(r'^api/datavalue/sn/(?P<serial_number>[a-zA-Z0-9-_\.]+)/(?P<today>today)/$', views.api_get_data),    
    #url(r'^api/datavalue/sn/(?P<serial_number>[a-zA-Z0-9-_\.]+)/from/(?P<from>\d{4}\-\d{1,2}\-\d{1,2})/to/(?P<to>\d{4}\-\d{1,2}\-\d{1,2})/$', rest.DataValueForDevDetail.as_view()),
)